...
Прикладами чисел з рухомою комою можуть бути `3.23` і `52.3E-4`. Позначення E показує ступеня числа 10. У цьому випадку "52,3E-4" означає "52,3 * 10<sup>-4</sup>".
...
### Метод форматування (англ."The format method")
...
1.пунктуація,як апостроф,двокрапка тощо, не можно використовувати у змінних (variables),тільки підкреслення "_".Тому у прикладі замість грамотично вірного написання слова "ім'я" використано -"ім_я";
...
### Необроблений рядок (англ."Raw String")

Якщо вам потрібно вказати деякі рядки, де не обробляються  спеціальні символи, наприклад escape sequences, тоді вам потрібно вказати _"raw string"_(необроблений рядок), додавши до рядка префікс `r` або `R`.Необроблений рядок Python розглядає символ зворотної похилої риски ( \ ) як буквальний символ.Наприклад:
...
## Bідступи (англ. "Indentation")
Пробіли (англ."Whitespace")важливі в Python. Точніше *пробіли на початку рядка важливі*. Це називається _відступами_(англ._"indentation"_). Передні відступи (пробіли та табуляції) на початку логічного рядка використовуються для визначення рівня відступу логічного рядка, який, у свою чергу, використовується для групування statement.
