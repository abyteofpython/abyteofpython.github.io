...
Якщо вам потрібно вказати деякі рядки, де не обробляються  спеціальні символи, наприклад escape sequences, тоді вам потрібно вказати _"raw string"_(_необроблений_рядок), додавши до рядка префікс `r` або `R`.Необроблений рядок Python розглядає символ зворотної похилої риски ( \ ) як буквальний символ.Наприклад:
