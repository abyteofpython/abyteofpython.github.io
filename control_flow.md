...
Зауважте, що `range()` генерує послідовність чисел, але одне число за раз. Щоб побачити всю послідовність чисел відразу, напешіть `list(range())`, наприклад, `list(range(5))` призведе до "[0, 1, 2, 3, 4]". Списки (англ. "list") пояснюються в [главі про структури даних](./data_structures.md#data-structures).
