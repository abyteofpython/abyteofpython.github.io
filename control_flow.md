# Потік керування (англ."Control Flow")
...
## Оператор `if` (англ."The `if` statement")
...
if здогадка == число:
    ...
    print('(але ви не виграєте жодних призів!)')
...
Вітаємо, ви вгадали.
(але ви не виграєте жодних призів!)
Виконано
```
...
Далі ми порівнюємо здогадку користувача із заданим нами числом. Якщо вони рівні, ми друкуємо повідомлення про успіх. Зверніть увагу, що ми використовуємо відповідні рівні відступу, щоб повідомити Python, які рядки коду належать до якого блоку. Ось чому відступи настільки важливі в Python. Сподіваюся, ви дотримуєтеся правила «постійних відступів». Чи не так?
...
Для отримання додаткової інформації перегляньте офіційну документацію Python: <https://docs.python.org/3/tutorial/controlflow.html#match-statements>.
 Ви можете використовувати блок `if..elif..else` щоб зробити те саме,що і за допомогою оператора `switch`(і в деяких випадках використовуйте [словник](./data_structures.md#dictionary),щоб зробити це швидко)


## Оператор `while`(англ."The while Statement")
...
_Ми переміщуємо оператори `input` і `if` всередину циклу `while` і встановлюємо змінну `гра_запуститься` у значенні `True` перед циклом while. Спочатку ми перевіряємо, чи змінна `гра_запуститься`(гра_запуститься==True) має значення `True`, а потім продовжуємо виконувати відповідний *while-block*.

Після виконання цього блоку *while-block* (p.118-126)знову перевіряється умова, яка в даному випадку є змінною `гра_запуститься`. Якщо вона істинна, ми знову виконуємо блок while, інакше ми продовжуємо виконувати необов’язковий блок else, а потім переходимо до наступного рядкам коду._
...
_Після оператора `while` завжди йде вираз. Вираз — це щось, що може обчислити комп’ютер і завжди має результат `True` або `False`. (Наприклад, 3>5 є `False`, але 3 == 3 є `True`).
...
Усередині блоку `while` є блок `if elif else`. Лише блок `if здогадка == число: ` if guess == number: містить команду для зміни значення `гра_запуститься`:
...
Однак існує особливий випадок: якщо виконується блок while і всередині блоку while виконується команда ```break```, тоді блок else НЕ буде виконано. Команда ```break``` буде пояснена пізніше в цьому розділі._

> **Примітка для програмістів C/C++**
> Пам’ятайте, що цикл `while` може мати блок `else`.


## Цикл `for` (англ."The `for` loop")

Оператор `for..in` — це ще один оператор циклу, який *ітерує* послідовність об’єктів, тобто проходить через кожен елемент у послідовності. Ми детально розглянемо про [послідовності](./data_structures.md#sequence) у наступних розділах. Зараз вам потрібно знати, що послідовність — це впорядкований набір елементів.
...
Зауважте, що `range()` генерує послідовність чисел, але одне число за раз. Щоб побачити всю послідовність чисел відразу, викличте `list()` на `range()`, наприклад, `list(range(5))` призведе до «[0, 1, 2, 3, 4]». Списки (англ. "list") пояснюються в [главі про структури даних](./data_structures.md#data-structures).
...
Пам’ятайте, що блок «else» необов’язковий. Якщо він присутній, він завжди виконується один
раз після закінчення циклу for, якщо тільки не вказано оператора [break](#break-statement).
...
## Оператор break (англ."The break Statement") 
...
Довжина рядка становить : 24
Введіть щось : Коли робота виконана
Довжина рядка становить : 20
Введіть щось : І якщо ви хочете повесилитися на роботі:
...
## Оператор  `continue` (англ." The `continue` Statement") 
...
У цій програмі ми приймаємо введення від користувача, але обробляємо введений рядок, лише якщо він містить принаймні 3 символи. Отже, ми використовуємо вбудовану функцію `len`, щоб отримати довжину рядка, і якщо довжина менша за 3, ми пропускаємо решту операторів у блоці за допомогою оператора `continue`. В іншому випадку решта операторів у циклі виконуються, виконуючи будь-яку обробку, яку ми хочемо виконати тут.
